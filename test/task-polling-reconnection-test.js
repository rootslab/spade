...
/* 
 * Spade, test for polling task in normal mode.
 */
exports.test = function ( done, assertions ) {
    ...
    client.connect( null, function () {
        ...
        // push expected events
        evts.push( 'polling', 'hangup', 'offline', 'attempt' );

        log( '- run polling task, with reconnect option set to %s, hanghup timeout is set to %s secs.', inspect( true ), inspect( 2 ) );
        client.tasks.polling.run( intval, [ null, 'BANG!', 1000, true ] );

        log( '- now waiting %s secs to collect events..', inspect( 5 ) );

        setTimeout( function () {

            log( '- unmute Vapid server.' );
            vp.mute( false );

            // push expected events
            evts.push( 'connect', 'reply', 'authorized', 'reply', 'dbselected', 'scanqueue', 'ready', 'reply' );
