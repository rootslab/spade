            ...
            var cfg = improve( opt, spade_opt )
                ...
                , onParserMatch = function ( err_reply, data, fn ) {
                    ...
                    if ( lsub.on ) {
                        ...
                        if ( ! cmd ) {
                            // command is undefined, a pub/sub message was received or "OK" reply to QUIT command.
                            subs = libra.update( message[ 0 ], message[ 2 ] );
                            if ( ~ subs ) {
                        ...
                        if ( cmd.isSubscription ) {
                            // otherwise, command could be a subscription.
                            if ( lsub.channels + lsub.pchannels === 0 ) {
                                subs = libra.update( message[ 0 ], message[ 2 ] );
                                if ( subs ) me.emit( 'listen' );
                                return me.emit( 'message', message );
                            }
                            /*
                             * IMPORTANT NOTE: It is not always possible to mantain bindings between
                             * commands callbacks and messages received through pub/sub system, then
                             * in pubsub mode all commands replies are received as messages.
                             * For example:
                             * - subscribe( [ 'a', 'a', 'b', 'b', 'c', 'c' ] ) get 6 messages, 3 actual subscriptions
                             * - unsubscribe() gets 3 messages, 0 subscriptions
                             */
                            // cmd.zn( false, data, cmd.fn || fn );
                            libra.update( message[ 0 ], message[ 2 ] );
                            return me.emit( 'message', message );
                        }
                        // could be a select command, continue
                    }
