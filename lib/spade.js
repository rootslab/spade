            ...
            var cfg = improve( opt, spade_opt )
                ...
                , onParserMatch = function ( err_reply, data, fn ) {
                    ...
                    if ( err_reply ) {
                        // pop command
                        cmd = libra.pop();
                        cmd.zn( true, data, cmd.fn || fn );
                        // check if the client is quitting connection
                        return cmd.isQuit ? cocker.bye() : null;
                    }
                    /*
                     * If PubSub mode is currently active, get the message but
                     * don't pop command immediately, check first if it is a 
                     * message reply to a (un)subscription command or a message
                     * from a publisher or from PubSub system.
                     */
