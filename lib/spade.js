...
exports.Spade = ( function () {
        ...
        , Spade = function ( opt ) {
            ...
            var cfg = improve( clone( opt ), spade_opt )
                ...
                , onParserMatch = function ( err_reply, data, fn ) {
                    ...
                    var cmd = null
                        ...
                        , getReply = function () {
                            ...
                            if ( glen ) me.emit( err_reply ? 'error-reply' : 'reply', cmd, data, cmd.fn || fn );
                ...
                , processCommandQueue = function ( address ) {
                    ...
                    me.cacheready = true;
                    if ( logger.events.length ) me.emit( 'scanqueue', libra.cqueue.size() );
                ...
                , send = function ( ocmd ) {
                    ...
                    if ( ~ libra.push( ocmd ) ) {
                        ...
                        return me.ready ?
                               ...
                               logger.events.length ? me.emit( 'queued', ocmd, libra.cqueue.size() ) : null
                               ;
                    }
    ...
    sproto.initCache = function ( file_load_opt, cache_init_opt, cback ) {
        ...
        lua.init( function ( clist ) {
            ...
            clist.forEach( function ( ocmd, i, events ) {
                // check first for command encoding error from Syllabus mix-in.
                if ( ocmd.err ) {
                    return me.emit( 'error', ocmd );
                }
    ...
    sproto.initTasks = function ( events ) {
        var me = this
            ;
        require( './tasks/' )( me, events );
        return me.tasks;
    };
