                ...
                , onParserMatch = function ( err_reply, data, fn ) {
                    ...
                    var cmd = null
                        ...
                        , getMessageReply = function () {
                            /*
                             * NOTE: callback for a multiple (un)subscriptions command
                             * is executed only when the first reply will be received.
                             */
                            mtype = message[ 0 ];
                            msubs = message[ 2 ];
                    ...
                    // check if pubsub mode is currently active
                    if ( lsub.active ) {
                        // check message type
                        mtype = message[ 0 ];
                        if ( mtype === 'message' ) {
                        ...
                        return cmd.isSubscription ? getMessageReply() : null;
                    }
                    ...
                    if ( lmon.active ) {
                        // if cmd.isMonitor then execute callback else emit 'monitor' with String
                        return cmd && cmd.isMonitor ?
                               cmd.zn( false, data, cmd.fn || fn ) :
                               me.emit( 'monitor', message[ 0 ], formatters.monitor )
                               ;
                    }

                    if ( cmd.isSubscription ) return getMessageReply();
