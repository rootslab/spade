        ...
        , Spade = function ( opt ) {
            ...
            var cfg = improve( opt, spade_opt )
                ...
                , onConnectionEstablished = function ( address ) {
                     ...
                     */
                    // return password ? sendAuth( address, password, db ) : processCommandQueue( address );
                    return password ? sendAuth( address, password, db ) : sendSelect( address, db );
                }
                ...
                , sendSelect = function ( address, index ) {
                    var encode = syllabus.encode
                        ...
                        , db = toString( index ) === onum ? String( Math.abs( index ) ) : +index ? index : '0'
