            ...
            var cfg = improve( opt, spade_opt )
                ...
                , onConnectionEstablished = function ( address ) {
                    var key = address.host + ':' + address.port
                        ...
                        , security = osec[ key ]
                        , passwd = security && security.mandatory ? security.requirepass : null
                        ;
                    /*
                     * Check password, if authorization is not mandatory,
                     * then process command queue; otherwise, set the auth
                     * property with the encoded 'AUTH password' command.
                     */
                    if ( ! passwd ) processCommandQueue( address );
                    me.auth = syllabus.encode( 'AUTH', passwd, null, function ( err, data, fn ) {
                        if ( err ) {
                            /*
                             ...
                             */
                            return me.emit( 'authfailed', address, passwd, fn( data ) );
                        }
