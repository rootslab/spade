    ...
    var log = console.log
        ...
        , Spade = function ( opt ) {
            ...
            var cfg = improve( opt, spade_opt )
                 ...
                 * On AUTH error reply, Command queue will not be processed; it also
                 * happens when an AUTH command was sent, but the password is not
                 * required by Redis. The client will disconnect from server to force
                 * the sending of another AUTH command on re-connection.
                 */
                    ...
                    var encode = syllabus.encode
                        , onAuthReply = function ( err, data, fn ) {
                            var reply = fn( data )
                                ;
                            if ( err ) {
                                me.emit( 'authfailed', address, password, reply );
                                return cocker.bye();
                            }
                            me.emit( 'authorized', address, password, reply );
