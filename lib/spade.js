            ...
            var cfg = improve( opt, spade_opt )
                ...
                , onParserMatch = function ( err_reply, data, fn ) {
                    ...
                    if ( lsub.on ) {
                        ...
                        if ( ! cmd ) {
                            // if command is undefined, a message was received, or OK reply to a QUIT command.
                            subs = libra.update( message[ 0 ], message[ 2 ] );
                            return ~ subs ? me.emit( 'message', message ) : cocker.bye();
                        }
                        if ( cmd.isSubscription ) {
                            ...
                            /*
                             * IMPORTANT NOTE: the command callback is executed only for the first reply to
                             * (UN)SUBSCRIBE. Examples are: multiple subscriptions in a single command, or a
                             * unsubscription without arguments. However, all replies are considered and will
                             * be received as messages.
                             */
                            return me.emit( 'message', message );
                        }
                        // could be a select command
                    }
