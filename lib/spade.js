...
exports.Spade = ( function () {
        ...
        , Spade = function ( opt ) {
            ...
            var cfg = improve( clone( opt ), spade_opt )
                ...
                , onParserMatch = function ( err_reply, data, fn ) {
                    ...
                    var cmd = null
                        ...
                        , getReply = function ( is_err ) {
                            // pop command from queue
                            cmd = libra.pop();
                            // if no command is popped, handle reply as a message
                            if ( ! cmd ) return getMessage();
                            // execute callback
                ...
                , processCommandQueue = function ( address ) {
                    ...
                    me.cacheready = true;
                    if ( gerry.list.length ) me.emit( 'scanqueue', libra.cqueue.size() );
                ...
                , send = function ( ocmd ) {
                    ...
                    if ( ~ libra.push( ocmd ) ) {
                        ...
                        return me.ready ?
                               ...
                               gerry.list.length ? me.emit( 'queued', ocmd, libra.cqueue.size() ) : null
                               ;
                    }
    ...
    sproto.initTasks = function ( list ) {
        var me = this
            ;
        require( './tasks/' )( me, list );
        return me.tasks;
    };
